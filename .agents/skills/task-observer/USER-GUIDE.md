...
The skill is a multi-file bundle: `SKILL.md` plus a `references/` folder that is loaded on demand. To install it, put both into one folder, zip that folder, and upload it via Settings â†’ Capabilities in your Claude account. Uploading the bare SKILL.md alone results in a degraded install â€” the skill still works, but will tell you which files are missing (see the Installation section of the readme for other environments).
...
- `[your shared folder]/skill-observations/` â€” the observation log, the cross-cutting principles file, and an archive of resolved observations
- `[your shared folder]/skill-updates/` â€” staged versions of skill updates that are waiting for you to install them

Existing files in your shared folder are not modified by the observer. If you point the meta-skill at a folder that already contains client work or personal documents, those files stay where they are; the observer only reads from them when you ask Claude to use them in a task. Skill updates are also never installed automatically â€” they're staged in `skill-updates/` for you to review and install yourself.
