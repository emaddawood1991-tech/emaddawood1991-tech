...
description: >
  ...
  as "One Skill to Rule Them All" â€” trigger on this phrase too. IMPORTANT:
  this skill should be invoked at the start of every task-oriented session â€” if
  ...
  or harness-level session-start hook (see Recommended Activation Setup) â€”
  description-level matching alone is not enforceable.
---

# Task Observer â€” Continuous Skill Discovery & Improvement

**Created by Eoghan Henn / [rebelytics.com](https://rebelytics.com)** â€”
...
The links in this block are references for the human reader â€” executing
...
cwd inside an ephemeral checkout â€” a git worktree under
`.claude/worktrees/`, a temporary clone â€” is torn down with the checkout
...
## Reference files â€” load on demand, not up front

- `references/weekly-review.md` â€” the comprehensive review procedure
...
- `references/skill-authoring.md` â€” taxonomy details, licensing, attribution
  template, lean-content rule, confidentiality layers 2â€“5, principle
  propagation, live-file editing rules. Load before creating or editing any
  skill.
- `references/environments.md` â€” activation/config setup, compaction
...
(review triggers â†’ weekly-review; creating/editing a skill â†’
skill-authoring; setup/no-filesystem â†’ environments), load the file before
proceeding â€” never improvise the episode from this core file. If you notice
...
1. If `skill-observations/log.md` or `cross-cutting-principles.md` don't
   ...
   `never` if it doesn't exist â€” never write a date into it at setup; a
   ...
   on the stable project path first â€” state written to an ephemeral
...
3. Read `skill-observations/last-review-date.txt`. The value carries the
   ...
   has run yet. A missing file is abnormal (step 1 creates it) â€” recreate
   ...
   reviewed [in N days / yet] â€” run it now, or carry on with your task?")
...
5. Note the log's modification time. If modified in the last few hours,
   another session may be writing to it â€” re-read immediately before every
...
doing the work to discussing it** â€” user feedback in review phases is often
...
a skill and could make it better â€” problems, positive signals, or neutral
...
consistently fails to follow (convert to structural enforcement â€” checklist,
verification step, unskippable tool call â€” or remove it). Treat these as a
...
Append to the log **silently, within the same turn or the next** â€” never
...
must **write to the log** â€” not merely pause to ask yourself a question. Either
...
whether" is not enforcement. This is a hard checkpoint, not a suggestion â€” the
...
deliverable â€” `present_files`, a deck or PDF render, a staged skill file handed
to the user â€” or complete a task/todo batch, flush any pending observations to
...
1. *Pre-check:* read the actual log and find the highest existing number â€”
...
2. *Pre-write assertion:* immediately before appending, confirm the proposed
   ...
   meta-observation â€” it signals a parallel-session collision).

3. *Post-write verification:* after appending, count occurrences of the
   number; if >1, a parallel writer collided between check and write â€”
   ...
   your `>>`; your entry starts at the old line count + 1) â€” do NOT
   ...
   affected line to confirm the substitution actually took effect â€” a
...
**Log-write safety â€” never let a mutation span entry boundaries:** When
...
1. **Re-read and merge immediately before any write-back.** Any full-file
   ...
   snapshot â€” the write-back succeeds, the victim gets no error, and the
   ...
   the mutation, then â€” immediately before writing â€” re-read the live log
...
2. **Isolate the target entry, or anchor to a single line.** Either split
   ...
   isolation, and reassemble â€” OR, for a status-only edit, use a strictly
...
3. **Assert a structural invariant against the LIVE pre-write file.** Count
   ...
   snapshot â€” an invariant computed against a stale snapshot validates that
...
4. **Keep the pre-write backup.** Copy `log.md` before any programmatic
   ...
   above occurred â€” it turned a destructive bug into a non-event.

5. **Verify your entries SURVIVED, not just that they were written.** A
   successful append proves nothing an hour later â€” a concurrent session's
...
must verify survival, not just successful writes â€” in a concurrent erase,
...
`**Status:** OPEN` as its first field â€” this is mandatory at write time, not
...
**Issue:** [What happened â€” specific enough to understand weeks later
...
**Principle:** [The generalisable takeaway â€” the most important field.]
```
...
`**Reference file:**` line â€” an observation whose evidence dies with the
...
Principle must be fully generalised â€” no client names, domains, or details
...
When citing an observation by number â€” in conversation, in a review report,
or from within another observation â€” the number must come from the entry's
...
- **Search-tool line numbers are positional metadata, not IDs.** `grep -n`
  ...
  header first â€” scan backwards from the matched line to the nearest
...
- **Plausibility check (cheap second layer):** before quoting any
  observation number, compare it against the known counter range â€” the
...
**Open-source** â€” client-agnostic, methodology-driven, useful to other
practitioners. **Internal** â€” contains user/client/project specifics or
...
a resolved status MUST record its date â€” `ACTIONED (YYYY-MM-DD) â€” [what was
done]` / `DECLINED (YYYY-MM-DD) â€” [reason]` â€” and archival moves only
...
Archival is a read-filter-rewrite â€” the highest-risk mutation the log
...
updated/created | DECLINED (YYYY-MM-DD) = user decided not to pursue â€”
...
Default: at end of session, as a grouped summary â€” improvements grouped by
...
Observations" â€” an explicit user request that names the action, or
...
Issue â†’ Improvement â†’ Principle; each is typed; existing-skill items name
...
write time) â€” a statusless entry is invisible to any status-filtered review
...
number this session wrote and confirm each still exists exactly once â€” a
...
| Citing an observation number? | Only from its literal `### Observation N:` header â€” `grep -n` line numbers are positional metadata, not IDs; sanity-check against the known counter range |
| Open-source or internal? | Default open-source; the boundary is confidential |
| Small fix or substantial? | Additive â†’ apply directly; restructuring/new skill â†’ `references/skill-authoring.md` |
| Rewriting the log (archival/renumber/status)? | Backup â†’ re-read live and merge â†’ bounded mutation â†’ verify count against live pre-write file â†’ confirm own entries survived |
| Weekly review? | Trigger check at session start; procedure in `references/weekly-review.md` |
| No filesystem? | Handoff-doc mode â€” `references/environments.md` |
