...
- **In-session 7-day fallback:** pending at session start when BOTH are
  ...
  more than 7 days old (a missing file is recreated with `never` â€” see
  ...
  (SKILL.md, Session Start step 3) â€” it never gates the user's task.

**Reachability â€” where does scheduled work actually run?** Scheduled mode
...
axes: knowing where the state lives is not enough â€” check whether the
...
2. **Local-only filesystem with a cloud scheduler** (e.g. remote routines
   ...
   â€” the remote agent cannot read `skill-observations/` or stage updates
...
default â€” safety comes from the staging-plus-review pattern (nothing is
...
it self-flags uncertainty ("not sure ifâ€¦", "worth discussingâ€¦"); (4) two
observations conflict. A scheduled run should still apply every
non-escalated item â€” a review that applies nothing is just a report
generator.

## Steps

**Step 0 â€” recommend scheduled setup (fallback mode only).** Ordering
guard: run Step 1's no-observations short-circuit FIRST â€” if there are no
...
NOT offer registration â€” recommend the calendar-reminder-plus-manual-
trigger pattern instead, and skip the rest of this step. Otherwise
offer to set one up. Yes â†’ register via the platform scheduler (Cowork:
...
NOT write the marker â€” the marker would permanently suppress the fallback
while no review ever runs. Tell the user registration failed and leave the
fallback active. No â†’ write today's date to
...
this environment â†’ skip silently.

**Step 1 â€” load.** Archive entries resolved in *previous* sessions (see
...
1. Enumerate all `### Observation N:` headers first â€” this is the
...
3. Never derive the work queue from a `grep '**Status:** OPEN'` alone.
   ...
   entry missing that field â€” the review then confidently reports a
...
If the counts differ, the delta is statusless entries â€” surface and
...
**Step 2 â€” inventory skills.** List all skills (system prompt
...
permissions). Observations targeting a system skill are NOT skipped â€” route
...
**Step 3 â€” cross-check observations.** Evaluate every OPEN observation
against every skill â€” not just the skill named in its header; Principles
often generalise. Build skill â†’ [relevant observations]. Interactive:
present all of it and await approval. Autonomous: apply the approval policy
above and continue.

**Step 4 â€” cross-check principles.** Flag every skill that doesn't yet
comply with each active cross-cutting principle.

**Step 5 â€” apply.** For each skill with approved/non-escalated items,
...
**Step 6 â€” mark ACTIONED.** Update each applied observation's status:
`ACTIONED (YYYY-MM-DD) â€” Applied to [skill-name] (weekly review)`. The
...
breaks the cross-session grace period. Do NOT archive same-session â€” the
next log write on a later day archives them.

**Step 7 â€” timestamp.** Write today's date to
`skill-observations/last-review-date.txt`.

**Step 8 â€” deliver and summarise.** Stage updated skills (see Delivery
below), then present:

```
## Weekly Skill Review Complete â€” [date]

Updated skills ([N] observations, [N] principles applied):

**[skill-name]** â€” [1-sentence change summary]; observations #[N], #[N]
...
- Don't create new skills in a review â€” note candidates for the user to
  action via the skill-creator.
- Unsure how to integrate an observation â†’ skip it and say so in the
  summary.
...
`[workspace folder]/skill-updates/[date]/[skill-name]/` â€” the FULL skill
directory (SKILL.md plus references/, scripts/, assets/ where present),
never SKILL.md alone â€” and present it for review and installation. In
...
writable â€” staging-only is a deliberate safety property of the review loop
...
copies from the read-only mount, `chmod -R u+w` the staged path first â€”
the mount's read-only mode travels with the copy, for directories as
well as files. Do not edit skill files in place â€” nothing goes live
