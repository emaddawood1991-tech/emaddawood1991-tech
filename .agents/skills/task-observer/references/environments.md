...
instruction â€” suggest adding it if absent, creating the file if none
...
**Anti-pattern:** don't chain activation through another skill â€” load
...
edit blindly and never attempt to bypass the guard â€” a denial is the
...
governance-tracked config â€” many setups gate exactly those files.
...
trigger exists â€” a resumed session's opening message may not match the
...
share them with the user rather than fetching the pages â€” the skill's
...
down** â€” a premature offer is a minor interruption; a missing one is lost
work.
...
2. Then systematically read every section asking what skill gaps or
   candidates are *implied* but unstated â€” handoff docs carry signal beyond
