# Skill Authoring â€” taxonomy, licensing, confidentiality, editing rules
...
could go either way â€” strip specifics and generalise.
...
formal. They're working documents â€” keep them current, don't over-engineer.
...
**Embedded commands are pre-flight items too â€” execute before you ship.**
...
execution time; an embedded command runs verbatim, unattended, forever â€”
...
anti-patterns, or worked scenarios â€” bare rules get violated more than
...
files loaded on demand (progressive disclosure) â€” a skill loaded every
...
with credit â€” recommended default), **MIT** (code-heavy, permissive),
...
needed. All confidentiality sweeps still apply â€” other-client information
...
summary â€” e.g., "share and adapt for any purpose with credit."]
...
an issue on the skill's public repository â€” public feedback benefits every
...
repository actually exists â€” never write a reference to an artefact before
...
routing â€” a public-repo reference is wrong for that channel.
...
1. **Observation-level stripping** â€” open-source observations carry a fully
   generalised Principle (covered in SKILL.md).
2. **Pre-creation review** â€” before drafting/regenerating an open-source
...
3. **Post-draft sweep** â€” a separate re-read focused only on leakage:
   ...
   project.
4. **Structural principle** â€” when in doubt, remove. Slightly more generic
   beats slightly leaky.
5. **Cross-product re-identifiability sweep** â€” the final pass before any
   ...
   consolidating into composites. Run this mechanically â€” the author is the
   least reliable judge because they know the ground truth.

## Editing skills â€” always start from the live file
...
1. The live file is the authoritative source: in Claude Code,
   ...
   Do not edit skill files in place, in any environment â€” staging-only is
   what keeps the autonomous review safe.
2. Always base edits on a fresh read of the live file â€” never a workspace
...
4. Stage every update to
   `[workspace folder]/skill-updates/[date]/[skill-name]/` â€” the FULL
   skill directory (SKILL.md plus references/, scripts/, assets/ where
   present), never SKILL.md alone â€” and present it for review and
   ...
   in every environment â€” it's the review loop's safety property, not a
   ...
   mid-task). **Pre-delivery gate â€” two items, checked at the moment of
   ...
   is the `.skill` bundle â€” bare file links fail this gate even when all
   ...
   in-session checks, and read the archive listing back after zipping â€”
...
5. When seeding a staged copy by copying from the read-only mount, reset
   ...
   `cp --no-preserve=mode`) â€” the mount's read-only mode travels with
...
6. Match process rigour to the change: complex/open-source/uncertain design
   â†’ use the skill-creator if available; internal skills with requirements
   already established in conversation â†’ write directly, flagging
...
restructures), "nothing was lost" is checkable mechanically â€” but only with
a two-tier check:
...
3. For misses, substance-check via a distinctive mid-line substring before
   concluding loss â€” most misses are container artifacts (heading-level
...
defaults) as an explicit checklist â€” compression preferentially destroys
enforcement machinery because it reads as redundancy â€” and sweep any "pure
...
brief. Determine type early: open-source â†’ strip and generalise; internal â†’
include specifics freely; uncertain â†’ default open-source and let the user
...
The user chooses propagation timing: immediate (update all skills now â€” for
