...
1. **Identifies new skills for you** â€” it spots repeating patterns in your work and flags them as skill candidates, then helps you build them, so you get skills without staring at a blank page
2. **Improves your existing skills** â€” it notices corrections you make, preferences you express, and gaps in your current skills, then suggests specific updates

You work normally. It watches. Your skill library grows and gets better over time.

## The self-improving part

This is the detail that makes the task observer truly beautiful in my opinion. Because it runs during every session and observes all active skills â€” including itself â€” it captures improvements to its own methodology over time.

If it misses something, or if its observation format could be clearer, or if it's triggering in contexts where it shouldn't â€” it notices, and it logs that too. The skill that improves all your skills also improves itself.

## What it does

Task Observer monitors your work sessions and looks for three things:

1. **Corrections and adjustments** â€” if you adjust the AI's output or steer it in a different direction, that's a signal that a skill could be clearer or more complete
2. **Gaps no skill covers yet** â€” if you're doing something manually that could be systematised, the observer flags it as a candidate for a new skill
3. **Its own blind spots** â€” the observer watches itself too, capturing improvements to its own methodology as you use it
...
One honest boundary: the formal observation log and review cycle pay off most as your skill library and usage grow â€” many skills, parallel sessions, scheduled reviews. If you run a small setup with a handful of skills, your AI system's built-in memory features may cover much of the same ground with less overhead, and editing a skill directly is quick. The observer's value compounds with scale: adopt it early if you expect your library to grow, or come back to it when direct editing stops feeling manageable.
...
The skill is a small bundle: `SKILL.md` plus three files in `references/` that are loaded on demand (this keeps the always-loaded part lean). Installing only SKILL.md works, but runs degraded â€” the skill will tell you which files are missing.

**Get the files:** download the repo as a ZIP (Code â†’ Download ZIP) or clone it, and keep `SKILL.md` and the `references/` folder together.

**Claude (web interface, desktop app, mobile app, Cowork):** put `SKILL.md` and `references/` into one folder, zip that folder, and upload it via Settings â†’ Capabilities. The skill is then available in all chats and in Cowork tasks.
...
- Claude Code without desktop app â€” the methodology and format translate directly - users have reported seamless experiences with this.
...
- Other skills-compatible platforms (ChatGPT, Gemini CLI, Cursor, etc.) â€” the skill uses Claude-centric concepts like `<available_skills>` and skill-creator references that other systems would need to interpret or adapt. The SKILL.md format is cross-platform, but the content assumes Claude's architecture.
...
You're free to use, adapt, and redistribute â€” even commercially â€” as long as you give appropriate credit: Link to the original repo (https://github.com/rebelytics/one-skill-to-rule-them-all/) and name the author (Eoghan Henn / rebelytics.com).
...
- Tom DÃ¶rr: [https://x.com/tom_doerr/status/2072251701608784049](https://x.com/tom_doerr/status/2072251701608784049)
