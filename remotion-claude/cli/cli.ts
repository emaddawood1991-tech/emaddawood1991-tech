...
async function generateStory(options: GenerateOptions) {
  try {
    let claudeApiKey = options.claudeApiKey || process.env.ANTHROPIC_API_KEY;
    ...
    if (!claudeApiKey) {
      ...
      if (!response.claudeApiKey) {
        console.log(chalk.red("Claude API key is required. Exiting..."));
        process.exit(1);
      }
    ...
    if (!openaiApiKey) {
      const response = await prompts({
        type: "password",
        name: "openaiApiKey",
        message: "Enter your OpenAI API key (used for image generation):",
        validate: (value) => value.length > 0 || "API key is required",
      });
    ...
    const storySpinner = ora("Generating story with Claude...").start();
    ...
    storySpinner.succeed(chalk.green("Story generated with Claude!"));

    const descriptionsSpinner = ora("Generating image descriptions with Claude...").start();
...
yargs(hideBin(process.argv))
  .command(
    ...
    async (argv) => {
      await generateStory({
        claudeApiKey: argv["claude-api-key"],
